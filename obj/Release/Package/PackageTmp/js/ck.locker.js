var Locker = {
        ...
        if (data.length > 0) {
            ...
            $(data).each(function () {
                ...
                if (status != 'with-dealer') {
                    if (lockerActiveKit) {
                        if (lockerActiveKit["activeKitName"] === null || lockerActiveKit["kitID"] === null) {
                            html += canEdit ? '' : '| <a style=cursor:pointer id="shoppingcart"  onclick="Locker.ViewCart(' + this.ID + ');">View Detail</a> ';
                        }
                        else {                          
                           // var productName = lockerActiveKit["activeKitName"].replace(/-/g, "_");
                            html += canEdit ? '' : '| <a style=cursor:pointer id="shoppingcart"  onclick="Locker.ViewKitCart(' + this.ID + ',' + lockerActiveKit["kitID"] + ',&quot;' + lockerActiveKit["activeKitName"] +'&quot;);">View Detail</a> ';
                        }
                    }
                ...
                //Start
                html += '<br/><br/><span class="Zipp-image"  data-id="' + this.ID + '"> <a href="#" ZippBrowsed-image class="btn btn-primary" style="margin-top:20px;">Download File</a></span>';
                //End
    ...
    ViewCart: function (designid) {
        debugger;
        jQuery.noConflict();
        var url = "/Locker/ViewCart";
        $.ajax({
            type: "POST",
            url: url,
    ...
    ViewKitCart: function (designid, KitNo, ProductName) {
        jQuery.noConflict();
        var url = "/Locker/ViewKitCart";
        $.ajax({
            type: "POST",
            url: url,
            data: { "designid": designid, "KitNo": KitNo, "ProductName":ProductName },
            success: function (res) {
                $("#demoModal").html('');
                $("#demoModal").html(res);
                $('#demoModal').modal('show');
